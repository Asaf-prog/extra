...
def parse_tags(header_value: str | None, query_value: str | None = None) -> tuple[str, ...]:
    ...
    Multiple tags are unioned. Order is first-seen. Either source may be empty.
    """
    seen: set[str] = set()
    out: list[str] = []
...
def select_tool_names(tags: tuple[str, ...]) -> tuple[str, ...]:
    ...
    No tags -> all tools (documented default). With tags -> the union of the
    matching groups, plus the always-on ``debug`` tools. Unknown tags simply
    match nothing (so only the debug tools remain). Order is deterministic.
    """
    if not tags:
...
def auth_summary(headers: Mapping[str, str]) -> dict[str, object]:
    ...
    The Authorization **token is never included or returned**; only whether it is
    present and (if parseable) its scheme (e.g. ``Bearer``). ``X-Organization-Id``,
    ``X-Correlation-Id`` and ``X-MCP-Tool-Tag`` are not secrets and are returned
    as-is for debugging.
    """
