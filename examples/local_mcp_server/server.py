...
It serves Streamable HTTP at ``http://127.0.0.1:8765/mcp`` (override with the
env vars ``LOCAL_MCP_HOST`` / ``LOCAL_MCP_PORT`` / ``LOCAL_MCP_PATH``).

What it demonstrates:

* **Tool discovery** over a real (local) remote MCP connection.
* **tool_tags**: ``list_tools`` is computed *per request* from the
  ``X-MCP-Tool-Tag`` header (or a ``?tag=`` query param), so the platform's tag
  transport is verified end-to-end — only the matching tool groups (plus the
  always-on ``echo`` / ``server_info`` debug tools) are advertised.
* **before_mcp_request / auth headers**: ``server_info`` and ``echo`` report a
  *safe* summary of the received auth/identity headers (presence + scheme only),
  so header injection by ``HookedMCPAuth`` can be confirmed without exposing
  tokens.

Optional dev auth: set ``LOCAL_MCP_REQUIRE_AUTH=true`` to reject requests that
arrive without an ``Authorization`` header.

Logging is intentionally safe: it never prints tokens, secrets, or raw payloads.
"""
...
def _list_customers(_: dict) -> str:
    return _dump(_CUSTOMERS)


def _get_customer(args: dict) -> str:
...
def _customer_summary(_: dict) -> str:
...
def _get_doc(args: dict) -> str:
    doc = next((d for d in _DOCS if d["id"] == args.get("doc_id")), None)
    return _dump(doc) if doc else _dump({"error": "doc not found"})


def _echo(args: dict, headers: Mapping[str, str]) -> str:
    return _dump({"echo": args.get("text", ""), "received_auth": auth_summary(headers)})


def _server_info(_: dict, headers: Mapping[str, str]) -> str:
    return _dump(
        {
            "name": SERVER_NAME,
            "version": SERVER_VERSION,
            "groups": {g: list(names) for g, names in GROUPS.items()},
            "received_tags": list(parse_tags(auth_summary(headers).get("tool_tag"))),  # type: ignore[arg-type]
            "received_auth": auth_summary(headers),
        }
    )


# name -> (description, input schema, handler). Handlers take (args) or
# (args, headers) for the debug tools that report request metadata.
...
# ---------------------------------------------------------------------------
# MCP server with per-request, tag-aware tool listing
# ---------------------------------------------------------------------------
...
def build_server() -> Server:
    ...
    async def list_tools() -> list[types.Tool]:
        headers = _headers()
        _check_auth(headers)
        tags = parse_tags(auth_summary(headers).get("tool_tag"), _query_tag())  # type: ignore[arg-type]
