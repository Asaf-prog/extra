...
basic routing through the compiled hierarchy and a minimal recursive execution
skeleton. **No real LLM calls, prompts, sidecar, or tools yet** — those are wired
...
- Implement routing from the entrypoint through the hierarchy and a recursive
  execution skeleton that returns a response + trace.
- Define extension seams (interfaces) for prompt rendering, sidecar calls, and
...
- Sidecar client implementation (0006).
...
- [ ] Routing traverses the compiled hierarchy from the entrypoint.
- [ ] A request produces a response + trace through the execution skeleton.
- [ ] Seams for prompts/sidecar/tools exist as interfaces only.
